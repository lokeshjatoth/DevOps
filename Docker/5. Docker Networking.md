...
1. List the existing Docker networks on your system to see the defaults.
```
sudo docker network ls
```


2. Create a new custom bridge network named `app-network`.
```
sudo docker network create app-network
```


3. Run a detached Nginx container named `web-server` and attach it to your new `app-network`.
```
sudo docker run -d --name web-server --network app-network nginx
```

4. Run a second container using `busybox`, named `client-box`, on the same network to test connectivity. This container will be used to ping the web server.
```
sudo docker run -d --name client-box --network app-network busybox sleep 3600
```

5. From `client-box`, ping the `web-server` by its container name to test Docker’s internal DNS.
```
sudo docker exec client-box ping -c 2 web-server
```

6. Clean up the test containers now that DNS is verified.
```
sudo docker rm -f web-server client-box
```


7. Expose a new Nginx container named `web-server-exposed` to the host on port 8080.
```
sudo docker run -d --name web-server-exposed -p 8080:80 nginx
```


8. From your host, use `curl` to confirm that Nginx is accessible at `http://localhost:8080`.
```
curl http://localhost:8080
```

9. Run a container with no network access.
```
sudo docker run -dit --name no-net-container --network none busybox sh
```

10. Attempt to ping an external address from the `no-net-container`. This will fail, proving it has no network connectivity.
```
